...
Verbatim port of ``opensquilla.engine.routing.calibration``. The only
adaptation: ``opensquilla.paths.state_dir`` is replaced by a configurable
module-level state directory (``set_state_dir`` / ``get_state_dir``),
defaulting to ``~/.squilla_router_standalone``.
...
# Configurable state directory (replaces opensquilla.paths.state_dir).
_STATE_DIR = Path.home() / ".squilla_router_standalone"


def set_state_dir(path: str | os.PathLike[str] | None) -> None:
    """Override the directory used for ``router_calibration.json``."""
    global _STATE_DIR
    _STATE_DIR = Path(path) if path is not None else Path.home() / ".squilla_router_standalone"
...
def state_dir(*parts: str) -> Path:
    """Mirror of ``opensquilla.paths.state_dir`` over the configurable state root."""
    root = _STATE_DIR
