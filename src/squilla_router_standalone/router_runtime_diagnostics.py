...
Verbatim port of ``opensquilla.router_runtime_diagnostics`` (pure stdlib).
"""
...
def router_runtime_hint(kind: str) -> str:
    if kind == MACOS_LIBOMP_MISSING:
        return (
            ...
            "`brew install libomp`, then restart the router."
        )
    if kind == WINDOWS_VC_RUNTIME_MISSING:
        return (
            ...
            "reopen your terminal and restart the router."
        )
    if kind == ROUTER_ASSETS_MISSING:
        return (
            "Bundled SquillaRouter assets are missing. Reinstall the package "
            "with the `[ml]` extra, or if running from source, re-run the "
            "bundle copy into `models/v4.2_phase3_inference/`."
        )
    if kind == ROUTER_PYTHON_DEPENDENCY_MISSING:
        return (
            "Router Python dependencies are missing; install or reinstall the "
            "`[ml]` extra (numpy, scikit-learn, onnxruntime, lightgbm)."
        )
    if kind == ROUTER_NATIVE_DEPENDENCY_MISSING:
        return (
            "A native library required by the router failed to load. Reinstall "
            "the `[ml]` extra and verify platform runtime dependencies."
        )
    return (
        "Router runtime is unavailable; reinstall the `[ml]` extra or disable "
        "the router (strategy=heuristic)."
    )
...
def router_runtime_operator_message(error: BaseException | str | None) -> str:
    ...
    return (
        "Squilla router fallback active: local ML router runtime failed to load. "
        "Routing still runs with the safe heuristic fallback, but ML routing is "
        "disabled until the runtime is available. "
