...
Verbatim port of ``opensquilla.router_control``. The ``opensquilla.router_tiers``
import is repointed to the standalone package, and the replay-event helper
(``router_control_replay_event_from_payload``) returns ``None``: the turn-replay
machinery it fed lives in the engine and is out of scope for the standalone
routing library. Everything on the routing decision path (hold store, target
...
def router_control_replay_event_from_payload(
    ...
    *,
    ...
    """Standalone stub: turn-replay machinery is out of scope. Returns None."""
