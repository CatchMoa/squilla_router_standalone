"""Standalone adapter for the V4 Phase 3 model router bundle.

Verbatim port of ``opensquilla.squilla_router.v4_phase3``. Imports are
repointed to the standalone package, and ``default_bundle_dir`` resolves to
the bundle copied under ``squilla_router_standalone/models/``.
"""
...
def default_bundle_dir() -> Path:
    """Return the package-bundled V4 Phase 3 runtime asset directory."""
    return Path(__file__).resolve().parent.parent / "models" / "v4.2_phase3_inference"
        ...
        if not self._degraded_warned:
            ...
            log.warning(
                ...
                detail=(
                    ...
                    "default tier (source=v4_unavailable). Install the `[ml]` extra "
                    "and verify the bundle to restore ML routing, or set "
                    "require_router_runtime=true to fail fast."
                ),
            )
